...
export class ClientWorld implements IWorld {
  ...
  petBag(): PetBagView {
    return this.self ? this.self.petBag : EMPTY_PETBAG; // GDD v0.5 (Pets PET2): the transport pet's bag
  }
  ...
  market(): MarketView {
    return this.self ? this.self.market : EMPTY_MARKET; // Global Marketplace: the full board of listings
  }
...
function entityView(s: EntitySnap, x: number, z: number, facing: number): EntityView {
  return {
    ...
    phyDef: 0, magDef: 0, parry: 0, blockRatio: 0, berserkGauge: 0, // K6/Fase 3/Sistema 2: placeholders (stats privados de remotos não são exibidos)
...
function mergeSelf(v: EntityView, s: SelfSnap): EntityView {
  return {
    ...
    phyDef: s.phyDef, magDef: s.magDef, parry: s.parry, blockRatio: s.blockRatio, berserkGauge: s.berserkGauge, // K6/Fase 3/Sistema 2: stats autoritativos do jogador local
  };
}

const EMPTY_ABILITIES: ReadonlyArray<AbilityView> = [];
const EMPTY_INVENTORY: InventoryView = { capacity: 0, stacks: [], slots: [], equipment: [], activeSets: [] };
