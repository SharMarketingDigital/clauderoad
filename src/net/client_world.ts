...
function entityView(s: EntitySnap, x: number, z: number, facing: number): EntityView {
  return {
    ...
    phyDef: 0, magDef: 0, parry: 0, blockRatio: 0, // K6/Fase 3: placeholders (defesa/esquiva/block de remotos não é exibida)
...
function mergeSelf(v: EntityView, s: SelfSnap): EntityView {
  return {
    ...
    phyDef: s.phyDef, magDef: s.magDef, parry: s.parry, blockRatio: s.blockRatio, // K6/Fase 3: defesa+esquiva+block autoritativos do jogador local
  };
}
